...
const ModalPost = ({showModal, setShowModal, date}) => {
  ...
  //   (state) => state.diary
  // );
  const  diaries  = useSelector((state)=> state.diary.diaries.filter((v)=>v.calendarDate === date));
...
//     if(diaries[imagePath].length >=1 ) setFilteredImagePath(diaries[imagePath].filter((v)=>v.includes(date)))
//   }, [diaries[imagePath], date]);
